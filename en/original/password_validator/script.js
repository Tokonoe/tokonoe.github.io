...
const checkPass = function() {
	if(lowerCase && upperCase && eightChar && number) {
		...
		validation.innerHTML = 'Your password is securised';
	}
	else {
		validation.style.color = 'red';
		validation.innerHTML = 'Your password needs at least ';

		if(lowerCase === false) {validation.innerHTML += '| 1 lower character '}
		if(upperCase === false) {validation.innerHTML += '| 1 upper character '}
		if(number === false) {validation.innerHTML += '| 1 number '}
		if(eightChar === false) {validation.innerHTML += '| 8 characters '}
	}
}
