...
const numbers = function() {
	for(i = 0; i < numValues.length; i++) {
		if(event.target.id === numValues[i].id) {
			...
			else if(single.innerHTML.length >= 12 && single.innerHTML !== 'LIMITE DE CHIFFRE') {
				//if single digit limit met

				let val = single.innerHTML;						//store single
				single.innerHTML = 'LIMITE DE CHIFFRE';				//display msg
...
const operator = function() {
	for(i = 0; i < opeValues.length; i++) {
		if(event.target.id === opeValues[i].id && single.innerHTML !== 'LIMITE DE CHIFFRE' && single.innerHTML !== 'PAS UN NOMBRE') {
			let equal = false;
			let slice = 0;
			let ope = false;
...
const equal = function() {
	...
	if(byZero) {
		full.innerHTML += '=INFINI';
		single.innerHTML = 'INFINI';
	}
	else if(oneOpe) {
		full.innerHTML += '=PAS UN NOMBRE';
		single.innerHTML = 'PAS UN NOMBRE';
	}
...
const dot = function() {
	...
	//CHECK
	//check msg
	if(single.innerHTML === 'LIMITE DE CHIFFRE') {msg = true}
