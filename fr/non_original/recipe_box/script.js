...
const get_data = function() {
	...
	if(recipe_name === '') {recipe_name = 'Sans Titre'}
...
const add_recipe = function(name, ings) {
	...
	<dl class="recipe" id="recipe_${recipe_number_name}">
		...
		<dt class="ingredients">
			...
			<button class="edit_btn">Éditer</button>
		</dt>
	</dl>
	`
...
const edit_recipe = function() {
	...
	document.getElementById('title').innerHTML = 'Éditer la Recette';
	document.getElementById('confirm_recipe').innerHTML = 'Éditer la Recette';

	document.getElementById('recipe_name').value = recipes_names[parseInt(id[id.length-1])];
...
const hide_add_recipe_menu = function() {
	setTimeout(function(){
		document.getElementById('title').innerHTML = 'Ajouter une Recette';
		document.getElementById('confirm_recipe').innerHTML = 'Ajouter la Recette';
...
if(document.getElementById("recipe_list").innerHTML === "") {
	add_recipe('Tarte à la Citrouille', ['Purée de Citrouille', 'Lait Concentré Sucré', 'Oeufs', 'Épice pour Tarte à la Citrouille', 'Pâte à Tarte']);
	add_recipe('Spaghetti', ['Nouilles', 'Sauce Tomate', '(Optionnel) Boulettes de Viande']);
	add_recipe("Tarte à l'Oignon", ['Oignon', 'Pâte à Tarte', "Ça a l'air délicieux pas vrai?"]);
}
